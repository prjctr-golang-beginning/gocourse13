...
func collectPatientData() <-chan PatientData {
	out := make(chan PatientData)
	go func() {
		for i := 1; i <= 5; i++ {
			out <- generateRandomData(i)
		}
		close(out)
	}()
	return out
...
func main() {
	...
	// Створюємо канали для кожного типу аналізів
	bloodTestData := analyzeBloodTest(patientData)
	urineTestData := analyzeUrineTest(patientData)

	// Об'єднуємо результати з різних каналів
